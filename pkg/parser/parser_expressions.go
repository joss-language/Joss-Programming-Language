...
func (p *Parser) parseInterpolatedString(tok Token) Expression {
	raw := tok.Literal
	...
	i := 0
	for i < len(raw) {
		if raw[i] == '\\' && i+1 < len(raw) && raw[i+1] == '$' && i+2 < len(raw) && raw[i+2] == '{' {
			literalText := raw[start:i] + "${"
			if len(segments) > 0 && !segments[len(segments)-1].isExpr {
				segments[len(segments)-1].text += literalText
			} else if literalText != "" {
				segments = append(segments, segment{isExpr: false, text: literalText})
			}
			i += 3
			start = i
			continue
		...
		if raw[i] == '$' && i+1 < len(raw) && raw[i+1] == '{' {
			if i > start {
				litText := raw[start:i]
				if len(segments) > 0 && !segments[len(segments)-1].isExpr {
					segments[len(segments)-1].text += litText
				} else {
					segments = append(segments, segment{isExpr: false, text: litText})
				}
			}
			...
			exprStr := strings.TrimSpace(raw[exprStart:i])
			segments = append(segments, segment{isExpr: true, text: exprStr})
			if i < len(raw) && raw[i] == '}' {
				i++
			}
			start = i
	...
	if !hasExpr {
		...
		return &StringLiteral{Token: tok, Value: combined.String()}
	}
	...
	for idx, s := range segments {
		...
		if s.isExpr {
			subLexer := NewLexer(s.text)
			subParser := NewParser(subLexer)
			expr := subParser.parseExpression(LOWEST)
			if expr == nil {
				current = &StringLiteral{Token: tok, Value: ""}
			} else {
				current = expr
			}
		} else {
			current = &StringLiteral{Token: tok, Value: s.text}
		}

		if idx == 0 {
			if s.isExpr {
				emptyLit := &StringLiteral{Token: tok, Value: ""}
				root = &InfixExpression{
					Token:    Token{Type: DOT, Literal: ".", Line: tok.Line, Column: tok.Column},
					Left:     emptyLit,
					Operator: ".",
					Right:    current,
				}
