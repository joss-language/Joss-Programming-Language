...
`joss analyze [archivo.joss]` analyzes the input and files `.joss` under `app/` without executingthe application.The pipeline loads each file as a source unit, registers global declarations, and then parses each callable with its own scope.
...
- Undefined variables, redeclarations and unused locales.
...
- Fixed inference on first assignment and remapping support.
...
- Symbols for native classes and JP v2 plugins loaded by the project.

## Evidence and unknown information

The parser differentiates `unknown` from invalid and `mixed` .The returns from all core APIs have explicit metadata, but some entries do not match all runtime results (see [audit](DOCUMENTATION_AUDIT.md) ).`mixed` represents intentional polymorphism.A native API without parameter metadata does not produce a speculative arity error;a dynamic receiver does not produce a member error;`isset` and `empty` can query a missing variable.

## Output

Diagnostics use the `pkg/diagnostics` model:
...
architecture `pkg/analyzer` does not depend on `pkg/core` .The `pkg/core/analyzer.go` adapter builds your `Environment` from the actual built-ins, native classes and plugins registers.The CLI uses `analyzer.LoadProject` , so it preserves file, line, and column instead of concatenating ASTs.

## Known Boundaries

- The exhaustive return test covers blocks, ternaries, `match` with `default` and `try/catch` ;does not yet demonstrate mathematical loop termination.
- Parameters of many native APIs remain variadic/unknown to avoid arity errors;Their returns are explicit.
- No branch-sensitive refinement analysis, formal taint/escape, or database schema contracts.
- Joss will not have a source import graph: the project uses autoloading and a single declaration space.
