...
So far we have worked with variables that store a single piece of data at a time: a number, a name, or a boolean. But in real life data almost never comes isolated:
...
1. What is an **array**, how numbering works from scratch (indexes) and how to add elements.
2. What is a **map** (key-value dictionary) with curly brace syntax `{}` or PHP associative style `["clave" => valor]`.
...
In Joss (and in the vast majority of modern languages), **indexes start counting from zero (`0`)**, not from one:
...
1. **Creation**: They are delimited with square brackets `[` and `]`, separating the elements by commas: `["pan", "leche"]`.
...
By default, an array `array` can contain mixed types. If you want to guarantee that all elements are integers, you can use the parameterized syntax:
...
You can expand the elements of an existing array inside another new array prepending `...`:
...
Un array is perfect when the order of the elements matters (like a waiting list). But if you want to represent an entity with tagged properties (like a user), remembering that "name is at index 0 and mail is at index 1" is brittle and confusing.
...
3. **Access and modification**: Brackets are used with the name of the key in quotes: `$persona["nombre"]`.
...
Joss supports both the classic curly brace notation `{}` with colon `:` as the square bracket notation and thick arrow `=>`, including nested multidimensional maps:
...
Just like in arrays, you can spread and merge key-value pairs of one map inside another using the operator `...`:
...
In addition to the pipeline operator, Joss allows calling fluid methods directly on primitive values (`string`, `array`, `map`):
...
## 5. In-memory behavior: Copy or reference?

This is a fundamental concept in the Joss architecture:

- When you assign a number or text to another variable (`$b = $a`), the value is copied independently.
- Instead, **arrays** and **maps** are managed internally using pointers and shared structures (Go slices and maps).
...
> To duplicate an array without sharing future changes, use the function `merge`:
...
Some functions to manipulate arrays have Specific contracts in Joss that differ from languages like PHP or JavaScript:
...
In Joss you can serialize and deserialize JSON using both **maps notation with braces `{}`** and **associative arrays notation with brackets `[]` (`=>`)**, in addition to conventional indexed arrays. Both formats are supported at `json_encode` and `json_decode` (or `JSON::encode` and `JSON::decode`):
...
### With associative fixes using square brackets `[]` and `=>`:
...
- **`json_decode($cadenaJson)`**: Converts a JSON string to Joss collections: `{}` objects are deserialized to associative maps `map`, lists `[]` in arrays and integers preserve the type `int`.
- **`json_verify($cadenaJson)`**: Validates whether the text contains a syntactically correct JSON structure.
...
- Sample in terminal: `"Producto: Laptop | Precio: $1200.00 | Disponibles: 5"`.
...
Continues with: [Type system, inference and conversions](SISTEMA_TIPOS.md).
